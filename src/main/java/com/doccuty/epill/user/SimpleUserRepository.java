...
public interface SimpleUserRepository<T extends SimpleUser> extends CrudRepository<T, Long> {
	...
	@Query("SELECT new SimpleUser(u.id, u.firstname, u.lastname, u.username, u.password, u.salt, u.preferredFontSize, u.levelOfDetail, u.redGreenColorblind, u.weight) FROM SimpleUser u WHERE u.username LIKE :username")
...
//	T findByUsername(@Param(value = "username") String username);
	...
	@Query("SELECT u FROM SimpleUser u WHERE u.id = :id")
